db.system.js.save({_id: "fnmenuColor",
      value: function (data) { 

    var menuVar = db.clnGlobalSettings.findOne({companyId:data.companyId}, {menuColor:1});
    if (menuVar) {
        db.clnGlobalSettings.update({companyId:data.companyId}, {$set:{menuColor:data.menuColor}});
        db.clnActiveUserData.update({userLoginId:data.userLoginId}, {$set:{menuColor:data.menuColor}});
        return "success";
    } else {
        db.clnGlobalSettings.insert({companyId:data.companyId, activeFlag:1, createdDate:Date(), updatedDate:Date(), crmId:data.userLoginId, urmId:data.userLoginId, menuColor:data.menuColor});
        db.clnActiveUserData.update({userLoginId:data.userLoginId}, {$set:{menuColor:data.menuColor}});
        return "success";
    }
