...
value:function(companyId, statusType, pageNumber, nPerPage, searchKey) {
	...
	data.type = "regex";
		data.orderFroms = db.clnTrainingRequest.find({$or:[{"status" :{$regex:new RegExp(searchKey,'i')}}, {"requesteeDetails.eMail" :{$regex:new RegExp(searchKey,'i')}}, {"customCompanyCode" :{$regex:new RegExp(searchKey,'i')}}, {"requesteeDetails.type" :{$regex:new RegExp(searchKey,'i')}}], companyId:ObjectId(companyId), "orderDetails.userInfo.status":{$in:statusType}}).skip(
    pageNumber > 0 ? ((pageNumber-1)*nPerPage) : 0).sort({customCompanyCode:-1}).limit(nPerPage).toArray();
	}
    return data;
}});
