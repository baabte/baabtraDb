...
db.system.js.save({_id: "fnViewFeedbackRequests",
    ...
    var pendingFeedback = db.clnFeedbacks.find({targetList:{$in:[ObjectId(rmId)]},companyId:ObjectId(companyId),userResponse:{$not:{$elemMatch:{rmId:ObjectId(rmId)}}},validUntil:{$gt:ISODate()}},{title:1,description:1,createdDate:1}).sort( {createdDate: -1}).toArray();
